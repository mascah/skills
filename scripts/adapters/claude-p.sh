...
read -r cid head ids work < <(python3 -c 'import json,sys; c=json.load(open(sys.argv[1])); print(c["id"], c["checkout"]["head"], "-".join(c["order"]), json.dumps(c["order"]))' "$contract")
wt=.worktrees/$ids
...
python3 - "$attempt" "$tested" "$contract" <<'EOF'
...
r.update(schema=1, contract=c["id"], work=c["order"])   # identity comes from the contract, not the agent's typing
