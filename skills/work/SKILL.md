...
On interruption, inspect current checkout, plan progress, and evidence before continuing. Small work inspects the partial diff directly; a bounded or large unit instead reads `git status`/`git diff --stat` and `.grove-run/ledger.md` for the last ledgered attempt, not the diff itself. A late notification for an attempt already on the ledger does not reopen it; a return whose attempt identity is unrecorded or unmatched, or a worker gone idle without a report, is investigated per the controller loop above before anything is accepted. Reconcile changed assumptions and invalidated checks. Never treat an agent exit or a checked task as proof of acceptance.
